...
function instrumentArtFixupStaticTrampolines () {
  const pattern = '7f0f00f9:1ffcffff';

  let baseOfFunction = null;
  const libart = Process.getModuleByName('libart.so');
  const symbols = libart.enumerateSymbols();
  for (const symbol of symbols) {
    if (symbol.name === '_ZN3art11ClassLinker26VisiblyInitializedCallback29AdjustThreadVisibilityCounterEPNS_6ThreadEl') {
      baseOfFunction = symbol.address;
    }
  }

  // const baseOfFunction = Module.findExportByName('libart.so', '_ZN3art11ClassLinker26VisiblyInitializedCallback29AdjustThreadVisibilityCounterEPNS_6ThreadEl');
  const sizeOfFunction = 8192;
  // console.log("u wot m8: " + baseOfFunction);

  if (baseOfFunction) {
    const matches = Memory.scanSync(baseOfFunction, sizeOfFunction, pattern);
    // console.log("got " + matches.length + " matches for findingXZR");
    if (matches.length === 1) {
      Interceptor.attach(matches[0].address, function () {
        // console.log("baked potatoes: " + this.context.x27.add(0x18).readU64());
  ...
  const fixupImpl = libart.findExportByName('_ZN3art11ClassLinker22FixupStaticTrampolinesEPNS_6ThreadENS_6ObjPtrINS_6mirror5ClassEEE');
  if (fixupImpl === null) {
    return;
  }
